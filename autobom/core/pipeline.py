...
def aggregate(bom_lines: list[BomLine], il_lines: list[IlLine]) -> list[Part]:
    ...
    attributes: dict[str, BomLine] = {}

    for il_line in il_lines:
        for bom_line in by_assembly.get(il_line.assembly_key, ()):
            quantity = bom_line.item_quantity * il_line.assembly_quantity
            totals[bom_line.part_number] = (
                totals.get(bom_line.part_number, Decimal(0)) + quantity
            )
            attributes.setdefault(bom_line.part_number, bom_line)

    parts = [
        Part(
            quantity=total,
            part_number=part_number,
            thickness_label=classify_thickness(attributes[part_number].thickness),
            size=format_size(attributes[part_number].width, attributes[part_number].height),
            fits_trumpf=fits_trumpf(
                attributes[part_number].width, attributes[part_number].height
            ),
        )
...
def process(bom_paths: list[Path], il_paths: list[Path]) -> ProcessResult:
    ...
    for path in bom_paths:
        bom_lines.extend(parse_bom(path, issues))
    ...
    for path in il_paths:
        il_lines.extend(parse_il(path, issues))
