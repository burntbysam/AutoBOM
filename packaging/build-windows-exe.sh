#!/usr/bin/env bash
# Cross-build AutoBOM.exe on Linux.
#
...
# emits a genuine PE executable. Output: dist/AutoBOM.exe
#
# Native Windows (the CI job in .github/workflows/release.yml) remains the
# reference build. Use this for a fast local exe without a Windows machine.
set -euo pipefail

PYTHON_VERSION="${PYTHON_VERSION:-3.11.13}"
PBS_TAG="${PBS_TAG:-20250818}"
export WINEPREFIX="${WINEPREFIX:-$HOME/.wine-autobom}"
export WINEARCH=win64
export WINEDEBUG=-all

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
WORK="${WORK:-/tmp/autobom-winbuild}"
...
cd "$ROOT"
rm -rf build dist
wine 'C:\Python311\Scripts\pyinstaller.exe' --noconfirm packaging/autobom.spec

echo "==> smoke test"
wine dist/AutoBOM.exe --version

echo
echo "Built: $ROOT/dist/AutoBOM.exe"
file dist/AutoBOM.exe
